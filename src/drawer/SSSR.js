...
class SSSR {
  ...
  static getRings(graph, experimental = false) {
    ...
    for (let i = 0; i < connectedComponents.length; i++) {
      ...
      const ccAdjacencyMatrix = graph.getSubgraphAdjacencyMatrix([...connectedComponent])
      ...
      const { d, pe, pe_prime } = SSSR.getPathIncludedDistanceMatrices(ccAdjacencyMatrix)
      const c = SSSR.getRingCandidates(d, pe, pe_prime)
      const sssr = SSSR.getSSSR(c, d, ccAdjacencyMatrix, pe, pe_prime, arrBondCount, arrRingCount, nSssr)
